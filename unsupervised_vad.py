...
def vad_help():
	...
	print 3*' ','Usage:'
 	print 3*' ','python unsupervised_vad.py <OPTIONS> <INPUT> <OUTPUT>'+'\n'
	print 3*' ','OPTIONS:'
...
def frame_power(xframes):
    return np.mean(np.diagonal(np.dot(xframes,xframes.T)))

def geomean(val_array):
	"""Geometric mean function to be used in computing the 
        Spectral Flatness measure of a frame (GM/AM)"""
	p=0
	for i in val_array:
		p+=np.log(i)
